...
export default async function handleWelcomeMessage(interaction) {
  ...
  switch (subcommand) {
    case 'set': {
      ...
      await patchWebhook(clientId, token, createEmbed(
        ...
        'Your new welcome message has been saved.',
        [
    ...
    case 'set-dm': {
      ...
      await patchWebhook(clientId, token, createEmbed(
        'DM Message Updated',
        'Your new DM message has been saved.',
        [
          { name: 'Message', value: `\`\`\`${message}\`\`\`` },
          { name: 'Available Variables', value: '`{username}` `{server}` `{memberCount}`' }
        ]
    ...
    case 'preview': {
      await patchWebhook(clientId, token, createEmbed(
        ...
        'This is the raw template that will be used for welcome messages:',
        [{ name: 'Template', value: `\`\`\`${config.welcomeMessage}\`\`\`` }]
      ));
      break;
    }
