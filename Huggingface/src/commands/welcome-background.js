...
import { invalidateWelcomeBgCache } from '../canvas/welcomeCard.js';
...
export default async function handleWelcomeBackground(interaction) {
  ...
  switch (subcommand) {
    ...
    case 'upload': {
      ...
      if (config.welcomeCustomBackgrounds.length >= MAX_CUSTOM_SLOTS && !config.welcomeCustomBackgrounds.includes(name)) {
        const fields = [{ name: 'Current Slots', value: config.welcomeCustomBackgrounds.join(', ') || 'None', inline: false }];
      ...
      try {
        ...
        const bucketPath = `guilds/${guildId}/assets/backgrounds/welcome/${name}.png`;
        
        // Write standard cropped PNG out to storage
        await writeBinary(bucketPath, processedBuffer, 'image/png');
        
        if (!config.welcomeCustomBackgrounds.includes(name)) {
          config.welcomeCustomBackgrounds.push(name);
        }
    ...
    case 'switch': {
      ...
      if (DEFAULT_BACKGROUNDS.includes(name) || config.welcomeCustomBackgrounds.includes(name)) {
        config.welcomeBackground = name;
        await writeJSON(`guilds/${guildId}/config.json`, config);
        invalidateWelcomeBgCache(guildId, name);
        await patchWebhook(clientId, token, createEmbed('Background Switched', `Welcome background set to **${name}**.`));
        return;
      }
      
      const fields = [
        { name: 'Available Defaults', value: DEFAULT_BACKGROUNDS.join(', '), inline: true },
        { name: 'Your Custom Slots', value: config.welcomeCustomBackgrounds.join(', ') || 'None', inline: true }
      ];
    ...
    case 'delete': {
      ...
      try {
        ...
        config.welcomeCustomBackgrounds = config.welcomeCustomBackgrounds.filter(s => s !== name);
        
        if (config.welcomeBackground === name) {
          config.welcomeBackground = 'default1';
        }
        ...
        console.error('[Welcome Background Delete Error]:', err);
    ...
    case 'list': {
      const fields = [
        { name: 'Active Background', value: `\`${config.welcomeBackground}\``, inline: true },
        { name: 'Slots Used', value: `${config.welcomeCustomBackgrounds.length}/${MAX_CUSTOM_SLOTS}`, inline: true },
        { name: 'Defaults Available', value: DEFAULT_BACKGROUNDS.join(', '), inline: false },
        { name: 'Your Custom Slots', value: config.welcomeCustomBackgrounds.join(', ') || 'None', inline: false }
      ];
      await patchWebhook(clientId, token, createEmbed('Welcome Backgrounds', 'Your configured welcome backgrounds:', fields));
      break;
    }
