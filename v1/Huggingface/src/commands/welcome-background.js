...
export default async function handleWelcomeBackground(interaction) {
  ...
  switch (subcommand) {
    ...
    case 'upload': {
      ...
      if (config.welcomeCustomBackgrounds.length >= MAX_CUSTOM_SLOTS && !config.welcomeCustomBackgrounds.includes(name)) {
        await patchWebhook(clientId, token, createEmbed(
          ...
          [{ name: 'Current Slots', value: config.welcomeCustomBackgrounds.join(', ') || 'None', inline: false }]
        ));
        return;
      }
      ...
      try {
        ...
        if (!config.welcomeCustomBackgrounds.includes(name)) config.welcomeCustomBackgrounds.push(name);
    ...
    case 'delete': {
      ...
      try {
        await deleteBucketFile(`guilds/${guildId}/assets/backgrounds/welcome/${name}.png`);
    ...
    case 'list': {
      await patchWebhook(clientId, token, createEmbed(
        'Welcome Backgrounds',
        'Your configured welcome backgrounds:',
        [
